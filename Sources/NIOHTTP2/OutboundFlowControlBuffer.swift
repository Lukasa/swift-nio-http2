...
internal struct OutboundFlowControlBuffer {
    ...
    internal mutating func processOutboundFrame(_ frame: HTTP2Frame, promise: EventLoopPromise<Void>?) throws -> OutboundFrameAction {
        ...
        case .data(let body):
            ...
            if self.streamDataBuffers.modifyValue(forStreamID: frame.streamID, { $0.dataBuffer.bufferWrite((.data(body), promise)) }) == nil {
        ...
        case .headers(let headerContent):
            ...
            let bufferResult = self.streamDataBuffers.modifyValue(forStreamID: frame.streamID) { (state: inout StreamFlowControlState) -> Bool in
    ...
    internal mutating func updateWindowOfStream(_ streamID: HTTP2StreamID, newSize: Int32) {
        ...
        self.streamDataBuffers.modifyValue(forStreamID: streamID) {
            let hadData = $0.hasPendingData
    ...
    internal mutating func nextFlushedWritableFrame() -> (HTTP2Frame, EventLoopPromise<Void>?)? {
        ...
        let nextWrite = self.streamDataBuffers.modifyValue(forStreamID: nextStreamID) { (state: inout StreamFlowControlState) -> DataBuffer.BufferElement in
...
extension StreamFlowControlState: PerStreamState { }
